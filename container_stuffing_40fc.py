...
def calculate_cost(weight, volume):
    if 20 <= weight <= 24 and 60<= volume <= 65:
...
def get_volume_class(volume_class):
    if volume_class == '40 Feet Container':
        return 65
    else:
...
def generate_report(data, weight_classes, selected_volume_class, include_cost=True):
    fulfilled_files = []
    unfulfilled_files = []
    ...
    volume_class = get_volume_class(selected_volume_class)
...
# Streamlit UI code
st.title('40 Feet Container Stuffing')
...
if uploaded_file:
    ...
    if st.button("Generate Report"):
        report, unfulfilled_report = generate_report(data, weight_classes, volume_class, include_cost)
